...
export const Follow = ({post, user_id, fetchPosts}) => {
  ...
  return <div>
        <h6>
            {post.user.name}
            {post.user.id !== user_id ? <button disabled={loading} style={{cursor: 'pointer'}} onClick={()=>onFollow(post.user_id, follower, filtered, user_id, fetchPosts, setLoading)} className="mx-1 btn btn-outline-secondary btn-sm border-0 p-1">{post.user.id !== user_id && follower !== user_id  ? 'follow' : 'followed'}</button> : ''}
        </h6>
  </div>;
};
